( function ( spqlib, $ ) {
	...
	spqlib.jqplot = ( function () {
		...
		var PROP_CHART_TITLE = 'chart.title',
			/* BARCHART*/
		 PROP_CHART_HEIGHT_AUTOMATIC = 'chart.height.automatic',
		 PROP_CHART_BAR_WIDTH = 'chart.bar.width',
		 PROP_CHART_BAR_MARGIN = 'chart.bar.margin',
		 PROP_CHART_BAR_PADDING = 'chart.bar.padding',
		 PROP_CHART_AXIS_X_LABEL = 'chart.axis.x.label',
		 PROP_CHART_AXIS_Y_LABEL = 'chart.axis.y.label',
		 PROP_CHART_AXIS_X_LABEL_FONT_SIZE = 'chart.axis.x.label.font.size',
		 PROP_CHART_AXIS_Y_LABEL_FONT_SIZE = 'chart.axis.x.label.font.size',
		 PROP_CHART_AXIS_X_FONT_SIZE = 'chart.axis.x.font.size',
		 PROP_CHART_AXIS_Y_FONT_SIZE = 'chart.axis.y.font.size',
		 PROP_CHART_DIRECTION = 'chart.direction',
		 PROP_CHART_LEGEND_SHOW = 'chart.legend.show',
		 PROP_CHART_LEGEND_LOCATION = 'chart.legend.location',
		 PROP_CHART_AXIS_X_ANGLE = 'chart.axis.x.angle',
		 PROP_CHART_AXIS_Y_ANGLE = 'chart.axis.y.angle',
		 PROP_CHART_AXIS_LABEL_MAX_LENGTH = 'chart.axis.label.max.length',
			/* BUBBLE CHART*/
		 PROP_CHART_LEGEND_COLUMN_ASSET = 'chart.legend.column.asset',
		 PROP_CHART_LEGEND_COLUMN_RADIUS = 'chart.legend.column.radius',
		 PROP_CHART_TOOLTIP_LABEL_PATTERN = 'chart.tooltip.asset.label.pattern',
		 PROP_CHART_TOOLTIP_LABEL_LINK_SHOW = 'chart.tooltip.asset.link.show',
		 PROP_CHART_TOOLTIP_LABEL_LINK_PATTERN = 'chart.tooltip.asset.label.link.pattern',
		 PROP_CHART_TOOLTIP_X_LABEL = 'chart.tooltip.x.label',
		 PROP_CHART_TOOLTIP_Y_LABEL = 'chart.tooltip.y.label',
		 PROP_CHART_TOOLTIP_R_LABEL = 'chart.tooltip.r.label',
		 PROP_CHART_TOOLTIP_X_VALUE_PATTERN = 'chart.tooltip.x.value.pattern',
		 PROP_CHART_TOOLTIP_Y_VALUE_PATTERN = 'chart.tooltip.y.value.pattern',
		 PROP_CHART_TOOLTIP_R_VALUE_PATTERN = 'chart.tooltip.r.value.pattern',
		 ...
		 colorscheme = {
	      ...
	      defaultBarChartVerticalOptions = {
			  ...
			  defaultBarChartHorizontalOptions = {
				// Only animate if we're not using excanvas (not in IE 7 or IE 8)..
				title: { text: '', show: true },
				animate: true,
				seriesDefaults: {
			 ...
			 defaultCutLongLabelFormatter = function ( format, value ) {
		...
		function defaultBarchartTooltipContentEditor( str, seriesIndex, pointIndex, plot ) {
			var label = plot.axes.xaxis.ticks[ pointIndex ],
				 html = "<span class='jqplot-tooltip-label'>" + label + "</span></br><span class='jqplot-tooltip-series-name'>" + plot.series[ seriesIndex ].label + ' = </span>',
				 value = '';
		...
		function defaultPiechartTooltipContentEditor( str, seriesIndex, pointIndex, plot ) {
			var label = plot.data[ seriesIndex ][ pointIndex ][ 0 ],
				 ...
				 html = spqlib.piechart.defaultPieChartTooltipContent( label, value );
			return html;
		}
		...
		function defaultDonutchartTooltipContentEditor( str, seriesIndex, pointIndex, plot ) {
			var label = plot.data[ seriesIndex ][ pointIndex ][ 0 ],
				 value = plot.data[ seriesIndex ][ pointIndex ][ 1 ],
				 html = spqlib.donutchart.defaultDonutChartTooltipContent( label, value, config );
			return html;
		}
	    ...
	    function drawBarChart( label, series, config ) {
			var chartId = config.divId,
			 direction = config.extraOptions[ PROP_CHART_DIRECTION ] || 'vertical',
			 s = {};
	    ...
	    function drawPieChart( label, series, config ) {
			  var options = getPieChartOptions( config ),
			   data = createSeries( label, series ),
			   plot1 = jQuery.jqplot( config.divId, [ data ], options );
			  plot1.config = config;
	    ...
	    function drawBubbleChart( label, series, config ) {
			  var options = getBubbleChartOptions( config ),
			   data = createBubbleSeries( label, series ),
			   graphId = config.divId,
			   plot = jQuery.jqplot( graphId, [ data ], options );
			  plot.config = config;
			  c[ graphId ] = plot;
		...
		function defaultDrawLegend( config, data ) {
			var graphId = config.divId,
			 first = config.extraOptions[ PROP_CHART_LEGEND_COLUMN_ASSET ] || 'Label',
			 second = config.extraOptions[ PROP_CHART_LEGEND_COLUMN_RADIUS ] || 'Radius';
			$( '#' + graphId + '-legend' ).html( '' );
		...
		function defaultBarChartDataClick( ev, seriesIndex, pointIndex, data ) {
			...
			var dataObject = plot.data[ seriesIndex ][ pointIndex ],
			 ...
			 label = isHorizontal ? dataObject[ 1 ] : dataObject[ 0 ],
			 value = isHorizontal ? dataObject[ 0 ] : dataObject[ 1 ],
			 html = ev.data.config.barChartTooltipContent ? ev.data.config.barChartTooltipContent.call( label, value, config ) : spqlib.barchart.defaultBarChartTooltipContent( label, value, ev.data.config, seriesIndex );
			if ( oldPointIndex != pointIndex ) {
				tooltip.show();
			} else {
				tooltip.toggle();
		...
		function defaultBubbleChartDataHighlight( ev, seriesIndex, pointIndex, data, radius ) {
			var graphId = $( ev.target ).attr( 'id' ),
			 legendIdSelector = '#' + graphId + '-legend',
			 plot = c[ graphId ],
			 config = plot.config,
			 chart_left = $( '#' + graphId ).offset().left,
			 ...
			 yValue = data[ 1 ],
			 rValue = data[ 2 ],
			 xLabel = config.extraOptions[ PROP_CHART_TOOLTIP_X_LABEL ] || 'x',
			 yLabel = config.extraOptions[ PROP_CHART_TOOLTIP_Y_LABEL ] || 'y',
			 rLabel = config.extraOptions[ PROP_CHART_TOOLTIP_R_LABEL ] || 'r',
				labelPattern = config.extraOptions[ PROP_CHART_TOOLTIP_LABEL_PATTERN ] || '{%s}',
			 showLink = config.extraOptions[ PROP_CHART_TOOLTIP_LABEL_LINK_SHOW ] || 'false',
			 labelLinkPattern = config.extraOptions[ PROP_CHART_TOOLTIP_LABEL_LINK_PATTERN ],
			 assetNameSpan = '',
			 assetNameText = spqlib.util.formatString( labelPattern, assetName );
			...
			var xValuePattern = config.extraOptions[ PROP_CHART_TOOLTIP_X_VALUE_PATTERN ],
			 yValuePattern = config.extraOptions[ PROP_CHART_TOOLTIP_Y_VALUE_PATTERN ],
			 rValuePattern = config.extraOptions[ PROP_CHART_TOOLTIP_R_VALUE_PATTERN ];
		...
		function defaultDonutChartDataHighlight( ev, seriesIndex, pointIndex, data ) {
			var chartId = ev.currentTarget.id,
			 config = spqlib.getById( chartId ).config,
			 chart = $( '#' + chartId )[ 0 ],
			 tooltip = $( '#' + chartId + ' .jqplot-highlighter-tooltip' ),
			 offsetLeft = chart.offsetLeft,
			 offsetTop = chart.offsetTop,
		...
		function getBarChartOptions( config ) {
			...
			if ( showLegend ) {
				options.legend.show = showLegend == 'true';
			}
		...
		function getPieChartOptions( config ) {
			var options = spqlib.util.cloneObject( defaultPieChartOptions ),
			 title = config.extraOptions[ PROP_CHART_TITLE ];
			if ( title ) {
				options.title.text = title;
			}
		...
		function getBubbleChartOptions( config ) {
			var options = spqlib.util.cloneObject( defaultBubbleChartOptions ),
			 op = config.extraOptions,
			 title = op[ PROP_CHART_TITLE ];
			if ( title ) {
				options.title.text = title;
			}
		...
		function onMouseWheel( event ) {
			...
			var plot1 = event.data,
			 config = plot1.config,
			 x = event.originalEvent.pageX - $( '#' + config.divId ).offset().left,
			 y = event.originalEvent.pageY - $( '#' + config.divId ).offset().top,
			 directionX = x / $( '#' + config.divId ).width() - 0.5,
			 directionY = y / $( '#' + config.divId ).height() - 0.5;
