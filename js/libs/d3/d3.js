!( function () {
	var d3 = {
		...
		d3_arraySlice = [].slice, d3_array = function ( list ) {
	...
	if ( d3_document ) {
		try {
			...
			d3_element_prototype.setAttribute = function ( name, value ) {
				d3_element_setAttribute.call( this, name, String( value ) );
			};
			d3_element_prototype.setAttributeNS = function ( space, local, value ) {
				d3_element_setAttributeNS.call( this, space, local, String( value ) );
			};
			d3_style_prototype.setProperty = function ( name, value, priority ) {
				d3_style_setProperty.call( this, name, String( value ), priority );
			};
		}
	}
	...
	function d3_map_escape( key ) {
		return ( key = String( key ) ) === d3_map_proto || key[ 0 ] === d3_map_zero ? d3_map_zero + key : key;
	}
	function d3_map_unescape( key ) {
		return ( key = String( key ) )[ 0 ] === d3_map_zero ? key.slice( 1 ) : key;
	}
	...
	d3_class( d3_Set, {
		has: d3_map_has,
		add: function ( key ) {
			this._[ d3_map_escape( key = String( key ) ) ] = true;
			return key;
		},
		remove: d3_map_remove,
		values: d3_map_keys,
	...
	d3_color.prototype.toString = function () {
		return String( this.rgb() );
	};
	d3.hsl = d3_hsl;
	function d3_hsl( h, s, l ) {
		return this instanceof d3_hsl ? void ( this.h = +h, this.s = +s, this.l = +l ) : arguments.length < 2 ? h instanceof d3_hsl ? new d3_hsl( h.h, h.s, h.l ) : d3_rgb_parse( String( h ), d3_rgb_hsl, d3_hsl ) : new d3_hsl( h, s, l );
	}
	...
	var d3_lab_K = 18,
		d3_lab_X = 0.95047, d3_lab_Y = 1, d3_lab_Z = 1.08883,
	...
	function d3_rgb( r, g, b ) {
		return this instanceof d3_rgb ? void ( this.r = ~~r, this.g = ~~g, this.b = ~~b ) : arguments.length < 2 ? r instanceof d3_rgb ? new d3_rgb( r.r, r.g, r.b ) : d3_rgb_parse( String( r ), d3_rgb, d3_hsl_rgb ) : new d3_rgb( r, g, b );
	}
	function d3_rgbNumber( value ) {
		return new d3_rgb( value >> 16, value >> 8 & 255, value & 255 );
	}
	function d3_rgbString( value ) {
		return String( d3_rgbNumber( value ) );
	}
	...
	function d3_xhr( url, mimeType, response, callback ) {
		...
		xhr.header = function ( name, value ) {
			...
			if ( value == null ) { delete headers[ name ]; } else { headers[ name ] = String( value ); }
			return xhr;
		};
		xhr.mimeType = function ( value ) {
			if ( !arguments.length ) { return mimeType; }
			mimeType = value == null ? null : String( value );
			return xhr;
		};
	...
	function d3_time_formatPad( value, fill, width ) {
		var sign = value < 0 ? '-' : '', string = String( sign ? -value : value ), length = string.length;
	...
	function d3_geo_clip( pointVisible, clipLine, interpolate, clipStart ) {
		return function ( rotate, listener ) {
			...
			var segments,
				buffer = d3_geo_clipBufferListener(), ringListener = clipLine( buffer ), polygonStarted = false, polygon, ring;
	...
	function d3_geo_clipCircle( radius ) {
		...
		function intersect( a, b, two ) {
			var pa = d3_geo_cartesian( a ), pb = d3_geo_cartesian( b ),
				n1 = [ 1, 0, 0 ], n2 = d3_geo_cartesianCross( pa, pb ), n2n2 = d3_geo_cartesianDot( n2, n2 ), n1n2 = n2[ 0 ], determinant = n2n2 - n1n2 * n1n2;
	...
	d3.geom.hull = function ( vertices ) {
		...
		function hull( data ) {
			...
			var upper = d3_geom_hullUpper( points ), lower = d3_geom_hullUpper( flippedPoints ),
				skipLeft = lower[ 0 ] === upper[ 0 ], skipRight = lower[ lower.length - 1 ] === upper[ upper.length - 1 ], polygon = [];
	...
	function d3_interpolateHcl( a, b ) {
		...
		return function ( t ) {
			return String( d3_hcl_lab( ah + bh * t, ac + bc * t, al + bl * t ) );
		};
	}
	...
	function d3_interpolateHsl( a, b ) {
		...
		if ( isNaN( bh ) ) { bh = 0, ah = isNaN( ah ) ? b.h : ah; } else if ( bh > 180 ) { bh -= 360; } else if ( bh < -180 ) { bh += 360; }
		return function ( t ) {
			return String( d3_hsl_rgb( ah + bh * t, as + bs * t, al + bl * t ) );
		};
	}
	...
	function d3_interpolateLab( a, b ) {
		...
		return function ( t ) {
			return String( d3_lab_rgb( al + bl * t, aa + ba * t, ab + bb * t ) );
		};
	}
	...
	d3_transitionPrototype.attr = function ( nameNS, value ) {
		...
		function attrTween( b ) {
			return b == null ? attrNull : ( b = String( b ), function () {
		...
		function attrTweenNS( b ) {
			return b == null ? attrNullNS : ( b = String( b ), function () {
	...
	d3_transitionPrototype.style = function ( name, value, priority ) {
		...
		function styleString( b ) {
			return b == null ? styleNull : ( b = String( b ), function () {
	...
	d3.svg.axis = function () {
		...
		function axis( g ) {
			g.each( function () {
				var g = d3.select( this ),
					scale0 = this.__chart__ || scale, scale1 = this.__chart__ = scale.copy(),
					...
					range = d3_scaleRange( scale1 ), path = g.selectAll( '.domain' ).data( [ 0 ] ), pathUpdate = ( path.enter().append( 'path' ).attr( 'class', 'domain' ),
					d3.transition( path ) );
		...
		axis.orient = function ( x ) {
			...
			orient = x in d3_svg_axisOrients ? String( x ) : d3_svg_axisDefaultOrient;
			return axis;
		};
		axis.ticks = function () {
	...
	var d3_time_scaleSteps = [ 1e3, 5e3, 15e3, 3e4, 6e4, 3e5, 9e5, 18e5, 36e5, 108e5, 216e5, 432e5, 864e5, 1728e5, 6048e5, 2592e6, 7776e6, 31536e6 ],
		...
		d3_time_scaleLocalFormat = d3_time_format.multi( [ [ '.%L', function ( d ) {
