( function ( spqlib, $ ) {
	...
	spqlib.d3 = ( function () {
		...
		var defaultMousemove = function ( d ) {
		 ...
		 defaultMouseout = function () {
		    d3.select( '#tooltip-treemap' ).classed( 'hidden', true );
			},

		 defaultMouseClickOnLeaves = function ( ev ) {
				var config = ev.data;
			 ...
			 selected = d3.select( ev.currentTarget ).classed( 'selected' );
		     ...
		     yPosition = ev.pageY + 5,
			 label = $( ev.currentTarget )[ 0 ].__data__.name,
			 value = $( ev.currentTarget )[ 0 ].__data__.value,
			 url = $( ev.currentTarget )[ 0 ].__data__.url,
			 spanContent = '';
				if ( url ) {
	    ...
	    function drawTreemap( res, config ) {
			...
			var margin = { top: 0, right: 0, bottom: 0, left: 0 },
			 width = $( '#' + config.divId ).outerWidth( true ),
			 grandparent = d3.select( '#' + config.divId ).append( 'div' )
					.attr( 'class', 'grandparent' );
			grandparent.html( '<span></span>' );
			var headerHeight = $( '#' + config.divId + ' .grandparent' ).outerHeight( true ),
			 height = $( '#' + config.divId ).outerHeight( true ) - margin.top - margin.bottom - headerHeight,
			 ...
			 svg = d3.select( '#' + config.divId ).append( 'svg' )
			 ...
			 infoBox = d3.select( '#' + config.divId ).append( 'div' )
				  ...
				  function wordWrap( text ) {
					  text.each( function () {
						var text = d3.select( this ),
						 ...
						 maxWidth = d3.select( this.previousElementSibling ).attr( 'width' ),
						 maxHeight = d3.select( this.previousElementSibling ).attr( 'height' );
						...
						while ( word = words.pop() ) {
						  ...
						  var tspanHeight = window.getComputedStyle( tspan.node(), null ).getPropertyValue( 'font-size' ).replace( 'px', '' ),
						   altezzaLibera = tspanHeight * ( lineNumber + 2 ) + 10 < maxHeight;
						  if ( !altezzaLibera ) {
							  line.push( '...' );
							  tspan.text( line.join( ' ' ) );
							  break;
