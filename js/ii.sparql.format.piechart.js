spqlib.piechart = ( function () {
	...
	*/
	spqlib.sparql2PieChart = function ( config ) {
		...
		this.util.doQuery( config.endpoint, config.endpointName, config.sparqlWithPrefixes, spqlib.piechart.render, config, spqlib.piechart.preQuery, spqlib.piechart.failQuery );
	};
	...
	my.render = function ( json, config ) {
		...
		var chartId = config.divId,
		 chart = spqlib.piechart.chartImpl().drawPieChart( labels, series, config );
		$( '#headertabs ul li a' ).on(
			'click',
			function ( event, ui ) {
				var tabDivId = $( this ).attr( 'href' ),
					 t = $( tabDivId ).find(
						'.sparqlresultformat-piechart' );
				t.each( function () {
					var id = $( this ).attr( 'id' ),
						 c = spqlib.getById( id );
					if ( c ) {
